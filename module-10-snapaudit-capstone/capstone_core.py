...
class CapstoneEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.debug("Loading Module 3 RAG systems")
            ...
            if pdf_path.exists():
                ...
                logger.info(f"Module 3 RAG loaded with {len(self._doc_chunks)} chunks")
        except Exception as e:
    ...
    def module1_route(self, text: str) -> Dict[str, Any]:
        logger.debug(f"Module 1 routing request: {text[:50]}...")
        if self.route_layer:
            try:
    ...
    def module3_retrieve(self, query: str, top_k: int = 3) -> Dict[str, Any]:
        logger.debug(f"Module 3 retrieval request: '{query}' with top_k={top_k}")
        if not self._bm25:
