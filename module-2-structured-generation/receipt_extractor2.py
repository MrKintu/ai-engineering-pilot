...
class ReceiptExtractorOllama(BaseReceiptExtractor):
    """Ollama-backed extractor — runs entirely on your local machine with vision support."""

    def __init__(self, model: str = OLLAMA_MODEL, vision_model: str = OLLAMA_VISION_MODEL, max_retries: int = 3) -> None:
        super().__init__(max_retries=max_retries)
        self.model = model
        self.vision_model = vision_model
        logger.info(f"Initialized Ollama extractor with text model: {model}, vision model: {vision_model}")

    def _call_llm(self, prompt: str) -> str:
        """Call Ollama text generation API."""
    ...
    def _call_llm_with_image(self, prompt: str, image_b64: str) -> str:
        """Call Ollama vision API with image input."""
        logger.debug(f"Calling Ollama Vision API with model: {self.vision_model}")
        response = requests.post(
        ...
        logger.debug(f"Ollama Vision response received: {output[:100]}...")
        return output

    def _extract_text_from_image(self, image_bytes: bytes, mime_type: str = "image/png") -> str:
        """Extract raw text from image using Ollama vision model."""
        logger.info(f"Extracting text from image ({mime_type}) using vision model: {self.vision_model}")
        ...
        try:
            raw_text = self._call_llm_with_image(ocr_prompt, image_b64)
    ...
    def extract_from_image(self, image_bytes: bytes, mime_type: str = "image/png") -> Receipt:
        """Extract a Receipt from a receipt image using local Ollama vision + text models."""
        logger.info(f"Starting local image extraction ({mime_type})")
        
        # Step 1: Extract text from image using vision model
        try:
            raw_text = self._extract_text_from_image(image_bytes, mime_type)
            if not raw_text:
                raise RuntimeError("No text extracted from image")
        except Exception as e:
            logger.error(f"Vision OCR step failed: {e}")
            raise RuntimeError(f"Failed to extract text from image: {e}") from e
        
        # Step 2: Use the same extraction logic as text-based extraction
    ...
    def extract_from_image(self, image_bytes: bytes, mime_type: str = "image/png") -> Receipt:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt == 1:
                ...
                raw = self._call_llm_with_image(prompt, image_b64)
            else:
                ...
                raw = self._call_llm_with_image(retry_prompt, image_b64)
...
if __name__ == "__main__":
    print("Testing ReceiptExtractor with Ollama (Local Vision + Text)...")
    
    # Test with Ollama (now supports vision!)
    ...
    # Test vision extraction (if you have an image)
    print("Testing ReceiptExtractor with Ollama Vision...")
    print("💡 To test vision extraction, uncomment the code below and provide an image path:")
    print("# with open('receipt.png', 'rb') as f:")
    print("#     image_bytes = f.read()")
    print("#     receipt = extractor.extract_from_image(image_bytes)")
    print("#     print(receipt)")
    
    print("\n" + "="*50 + "\n")
    
    print("Testing ReceiptExtractor with OpenAI (Fallback)...")
    ...
    print(f"2. Pull text model: ollama pull {OLLAMA_MODEL}")
    print(f"3. Pull vision model: ollama pull {OLLAMA_VISION_MODEL}")
    print("4. Set environment variables in .env:")
    print(f"   OLLAMA_BASE_URL={OLLAMA_BASE_URL}")
    print(f"   OLLAMA_MODEL={OLLAMA_MODEL}")
    print(f"   OLLAMA_VISION_MODEL={OLLAMA_VISION_MODEL}")
    print("\n🚀 Your local vision-capable receipt extractor is ready!")
