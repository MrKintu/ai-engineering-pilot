...
class DenseVectorRetriever:
    ...
    def __init__(
        ...
        logger.info(f"Initialized DenseVectorRetriever with collection '{collection_name}' and Ollama model '{embed_model}'")
    ...
    def _embed(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.debug(f"Successfully generated Ollama embedding for text (length: {len(text)})")
            return embedding
        except Exception as e:
    ...
    def _embed_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.debug(f"Successfully generated batch embeddings for {len(texts)} texts")
            return embeddings
        except Exception as e:
    ...
    def create_collection(self):
        ...
        if self.embedding_dim is None:
            ...
            logger.info("Determining embedding dimension from test embedding")
    ...
    def index_chunks(self, chunks: List[DocumentChunk]):
        ...
        logger.info(f"Starting indexing of {len(chunks)} chunks using Ollama batch embeddings")
        points = []
        batch_size = 10
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            for j, (chunk, embedding) in enumerate(zip(batch_chunks, embeddings)):
                points.append(PointStruct(id=i + j, vector=embedding, payload=chunk.to_dict()))
            
            if (i + batch_size) % 50 == 0 or i + batch_size >= len(chunks):
                logger.debug(f"Processed {min(i + batch_size, len(chunks))}/{len(chunks)} chunks")
...
def generate_answer_with_context(prompt: str, context_chunks: List[DocumentChunk], model: str = None) -> str:
    ...
    logger.debug(f"Generating answer using Ollama model '{model}' with {len(context_chunks)} context chunks")
...
Answer:"""
    ...
    try:
        logger.debug(f"Sending request to Ollama API at {OLLAMA_BASE_URL}")
        response = requests.post(
...
def _render_sidebar() -> Tuple[str, str, int]:
    ...
    with st.sidebar:
        ...
        st.markdown("- Ollama server running on `http://localhost:11434`")
        st.markdown("- Qdrant on `http://localhost:6333`")
        st.markdown("- Models: `embeddinggemma:latest` and `gemma3:latest`")
...
def _validate_pdf_path(pdf_path: str) -> Optional[Path]:
    ...
    try:
        ...
        st.success(f"✓ PDF validated: {len(reader.pages)} pages found")
...
def _render_hybrid_tab(hybrid_retriever: Any, top_k: int, tab_hybrid: Any):
    ...
    """Render hybrid search tab with generation."""
    with tab_hybrid:
            ...
            if st.button("Generate answer", type="secondary"):
                ...
                try:
                    ...
                    if results:
                        ...
                        logger.warning("No results found to generate answer")
                        st.warning("No results found to generate answer.")
                except Exception as exc:
                    logger.error(f"Answer generation failed: {exc}")
                    st.error(f"Answer generation failed: {exc}")


def rag_pipeline(query: str, retriever: Any, model: str = None) -> Dict[str, Any]:
    ...
    if not retrieved_chunks:
        logger.warning("No relevant chunks found for RAG pipeline")
        return {
            "query": query,
    ...
    logger.info(f"RAG pipeline completed successfully with {len(sources)} sources")
    return {
        "query": query,
        "answer": answer,
        "sources": sources
...
def main():
    logger.info("Starting Streamlit application with Ollama integration")
    ...
    _, dense, sparse, hybrid = _initialize_retrievers(str(resolved_pdf_path), collection_name)
    ...
    logger.info("Streamlit application with Ollama integration initialized successfully")
